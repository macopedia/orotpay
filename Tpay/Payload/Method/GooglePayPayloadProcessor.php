...
readonly class GooglePayPayloadProcessor implements PayloadProcessorInterface, PayloadSanitizerInterface
{
    ...
    {
        return $method === PaymentMethod::GOOGLE_PAY;
    }
    ...
    public function process(PaymentTransaction $paymentTransaction): array
    {
        ...
        if ($token === '') {
            throw new RuntimeException('Google Pay token not set');
        }

        return $payload + [
                'pay' => [
                    'groupId' => PayGroup::GOOGLE_PAY->value,
